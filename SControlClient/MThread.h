...
private:
	...
	void ThreadMain()
	{
		while (m_run)
		{
			WaitForSingleObject(m_event, INFINITE);  // 无限制的等待

			int ret = (*m_work)();  // 执行任务函数
			if (ret != 0)
			{
				delete m_work;
				m_work = NULL;
...
public:
	...
	bool Stop()
	{
		...
		m_run = false;
		int ret = WaitForSingleObject(m_thread, 100);  // 等100ms，让线程执行完
		printf("stop ret : %d\r\n", ret);
		if (ret == WAIT_OBJECT_0)  // 等到线程执行完了
		{
			return true;
		}
		return false;
	...
	bool IsFree()
	{
		if (m_run && (m_work == NULL))  // 当前线程是否是空闲的
		{
			return true;
		}
		return false;
...
public:
	...
	bool Invoke()
	{
		...
		for (size_t i = 0; i < m_vecThreads.size(); i++)
		{
			if (!m_vecThreads.at(i)->Start()) // 遍历线程，让线程跑起来
			{
				isOk = false;
				break;
			}
	...
	void DispatchWork(const CMWork& work)
	{
		for (size_t i = 0; i < m_vecThreads.size(); i++)
		{
			...
			{
				m_vecThreads.at(i)->Work(work);  // 派发一个任务
				break;
			}
		}
	}

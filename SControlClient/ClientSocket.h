...
public:
	...
	void InitSocket(const CString& ip, const CString& port)
	{
		...
		char strIP[256]{};
		if (WideCharToMultiByte(CP_ACP, 0, ip, -1, strIP, 256, NULL, NULL) == 0)
		{
	...
	int DealCommand()
	{
		...
		while (true)
		{
			...
			TRACE("1------------------tick = %lld\r\n", GetTickCount64()); // GetTickCount64 返回毫秒数
			...
			if (len > 0)
			{
				...
				memmove(buf, buf + len, index - len); // 将 index - len 长度的字节，从 buf + len 复制到 buf

...
private:
	...
	struct M
	{
		...
		int wParam;
		M(T _t){
			hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);//创建和现有事件描述符相同的新的事件描述符。
			t = _t;
			wParam = -1;
		}
...
private:
	...
	void ThreadMain()
	{
		DWORD transferred;           // 返回的字节数
		ULONG_PTR completionKey;     // 与完成端口的时候绑定的那个socket对应的自定义结构体PER_HANDLE_DATA
